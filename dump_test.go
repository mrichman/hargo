...
func TestDumpToWritesEntryDetails(t *testing.T) {
	...
	if err := DumpTo(&buf, NewReader(strings.NewReader(har))); err != nil {
...
func TestDumpToMalformedJSONReturnsError(t *testing.T) {
	var buf bytes.Buffer
	if err := DumpTo(&buf, NewReader(strings.NewReader(`{"log":{ BROKEN`))); err == nil {
...
func TestDumpToEmptyEntries(t *testing.T) {
	var buf bytes.Buffer
	if err := DumpTo(&buf, NewReader(strings.NewReader(harWith("")))); err != nil {
		t.Fatalf("DumpTo() error = %v", err)
	}
	out := buf.String()
...
func TestDumpToRealHarFixture(t *testing.T) {
	var buf bytes.Buffer
	if err := DumpTo(&buf, NewReader(openFixture(t, "test/en.wikipedia.org.har"))); err != nil {
		t.Fatalf("DumpTo() error = %v", err)
	}
	out := buf.String()
